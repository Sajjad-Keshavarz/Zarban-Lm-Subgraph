...
export function getMarketByAuxillaryToken(
  ...
  for (let i = 0; i < markets.length; i++) {
    ...
    if (market.outputToken && market.outputToken! == (auxillaryToken).toString()) {
...
export function updatePosition(
  ...
  if (deltaCollateral.notEqual(BIGINT_ZERO)) {
    let lenderPosition = getOpenPosition(event, urn, ilk, PositionSide.COLLATERAL);
  ...
  if (deltaDebt.notEqual(BIGINT_ZERO)) {
    let borrowerPosition = getOpenPosition(event, urn, ilk, PositionSide.BORROWER);
...
export function liquidatePosition(
  ...
  const borrowerPosition = getOpenPosition(event, urn, ilk, PositionSide.BORROWER)!;
  const lenderPosition = getOpenPosition(event, urn, ilk, PositionSide.COLLATERAL)!;
...
export function snapshotPosition(
  ...
  if (snapshot == null) {
    ...
    snapshot.balanceUSD = position.balance.toBigDecimal()
...
export function createTransactions(
  ...
  const zarAmount = deltaDebt.toBigDecimal().times(market.rateAcc).div(BIGDECIMAL_ONE_RAY);
  ...
  if (deltaDebt.gt(BIGINT_ZERO)) {
    ...
    // Calculate actual DAI minted using rate
    borrow.amount = BigInt.fromString(zarAmount.truncate(0).toString()); // Convert back to BigInt
    ...
    // Calculate actual DAI minted using rate
    repay.amount = BigInt.fromString(zarAmount.truncate(0).toString()); // Convert back to BigInt
